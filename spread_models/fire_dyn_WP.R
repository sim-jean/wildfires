...
for(t in 2:100){
  ...
  for(i in 1:nrow(init_fire)){
    ...
    if(init_fire[i,1] %in% c(2:side-1) & init_fire[i,2] %in% c(2:side-1)){ 
      try2[(init_fire[i,1]),  (init_fire[i,2]-1)]   <- max(try2[(init_fire[i,1]),  (init_fire[i,2]-1)], rbernoulli(1,0.3))
      try2[(init_fire[i,1]-1),(init_fire[i,2])]     <- max(try2[(init_fire[i,1]-1),(init_fire[i,2])], rbernoulli(1,0.3))
      try2[(init_fire[i,1]+1),(init_fire[i,2])]     <- max(try2[(init_fire[i,1]+1),(init_fire[i,2])], rbernoulli(1,0.3))
    ...
    # Propagation of items on rank= 1 and columns within
      ...
      try2[(init_fire[i,1]),  (init_fire[i,2]-1)] <- max(try2[(init_fire[i,1]),  (init_fire[i,2]-1)], rbernoulli(1,0.3))
      try2[(init_fire[i,1]+1),(init_fire[i,2])]   <- max(try2[(init_fire[i,1]+1),(init_fire[i,2])], rbernoulli(1,0.3))
      try2[(init_fire[i,1]+1),(init_fire[i,2]+1)] <- max(try2[(init_fire[i,1]+1),(init_fire[i,2]+1)], rbernoulli(1,0.3))
    }
