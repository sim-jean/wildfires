...
landscape[[1]][floor(runif(1,min=0,max=side+1)),floor(runif(1,min=0,max=side+1))] <- 1
sum(landscape[[1]])
pheatmap::pheatmap(landscape[[1]], cluster_rows=F, cluster_cols=F)
for(t in 2:400){
...
pheatmap(try2, cluster_rows=F,cluster_cols = F)
...
for(i in 1:nrow(init_fire)){
 ...
 # try2[(init_fire[i,1]-1),(init_fire[i,2]-1)]   <- max(try2[(init_fire[i,1]-1),(init_fire[i,2]-1)], rbernoulli(1,0.05))
  try2[(init_fire[i,1]),  (init_fire[i,2]-1)]   <- max(try2[(init_fire[i,1]),  (init_fire[i,2]-1)], R?rbernoulli(1,0.05))
 # try2[(init_fire[i,1]+1),(init_fire[i,2]-1)]   <- max(try2[(init_fire[i,1]+1),(init_fire[i,2]-1)], rbernoulli(1,0.05))
# Same column
...
# Column after
  # try2[(init_fire[i,1]-1),(init_fire[i,2]+1)]   <- max(try2[(init_fire[i,1]-1),(init_fire[i,2]+1)], rbernoulli(1,0.05)) # REMOVED BECAUSE ANGLE CELL
  try2[(init_fire[i,1]),(init_fire[i,2]+1)]     <- max(try2[(init_fire[i,1]),(init_fire[i,2]+1)],  rbernoulli(1,0.05))
  # try2[(init_fire[i,1]+1),(init_fire[i,2]+1)]   <- max(try2[(init_fire[i,1]+1),(init_fire[i,2]+1)], rbernoulli(1,0.05)) # SAME
  }
